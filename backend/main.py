...
def _generate_final_response(data: GenerateFinalRequest) -> GenerateFinalResponse:
    ...
    model, _ = ai_manager.resolve_generation_model(model, data.image_base64, data.ocr_text or "")
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Локальная модель не смогла подготовить итоговый ответ: {exc}",
        ) from exc
...
def _build_draft_reply(
    ...
    topic_map = {
        "Проценты / кредит наличными": "Разберусь, почему начислились проценты по кредиту наличными, и поясню, как это связано с графиком платежей.",
        "Проценты / кредитная карта": "Проверю, почему произошло списание процентов, и коротко поясню логику расчета.",
        "Проценты / кредит": "Разберусь, почему появились проценты по кредиту, и поясню, какие даты и операции на это повлияли.",
        "Дебетовая карта / кэшбэк": "Проверю детали по дебетовой карте и поясню, как в вашем случае работают категории и начисление кэшбэка.",
        "Дебетовая карта / переводы и лимиты": "Проверю детали по дебетовой карте и подскажу, как в вашем случае работают переводы, лимиты или комиссии.",
        "Вклад / проценты": "Разберусь по вкладу и поясню, как считаются проценты и от чего зависит итоговый доход.",
        "Вклад / пополнение и закрытие": "Проверю детали по вкладу и поясню, как здесь работают пополнение, досрочное закрытие или снятие средств.",
        "Накопительный счет / проценты": "Проверю детали по накопительному счету и поясню, как ставка и остаток влияют на начисление процентов.",
        ...
        "Премиум / подписка": "Проверю детали сервиса и поясню, откуда появилось списание или изменение.",
        "Кэшбэк / уровень сервиса": "Сверю детали программы и объясню, как именно учитываются операции в вашем случае.",
        "Переводы / СБП / MCC": "Проверю тип операции и поясню, как она учитывается в вашем случае.",
        "Кредит / рефинансирование": "Посмотрю на контекст запроса и поясню, какие варианты здесь реально можно рассмотреть.",
        "Страхование": "Проверю, о каком именно страховом сценарии идет речь, и дальше поясню детали.",
    ...
    facts_part = ""
    if knowledge_facts:
        facts_part = " Проверю детали по обращению и сформулирую ответ без неподтвержденных выводов."
